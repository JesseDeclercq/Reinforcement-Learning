...
import math
import numpy as np
import Box2D                            # the physics engine for the game
...
import pyglet
from SyntheticImage import SyntheticImage as SI
...
class CarRacing(gym.Env, EzPickle):
    ...
    def _create_obstacles(self):
        ...
        obstacle_interval = np.floor((len(self.track) - 1)/(self.n_obstacles))    # after how many track vertices must a obstacle appear.
        count = 0
        ...
        for i in range(len(self.track)):
            if i % obstacle_interval == 0 and count < self.n_obstacles and i > 1:
                ...
                self.obstacle_poly.extend([road1_l[0], road1_l[1], 0,
                                           road1_r[0], road1_r[1], 0,
                                           road2_r[0], road2_r[1], 0,
                                           road2_l[0], road2_l[1], 0])

                self.obstacles_pos[count-1,:,0] = np.array([road1_l[0],
                                                            road1_r[0],
                                                            road2_r[0],
                                                            road2_l[0]])

                self.obstacles_pos[count-1,:,1] = np.array([road1_l[1],
                                                            road1_r[1],
                                                            road2_r[1],
                                                            road2_l[1]])
    ...
    def _create_track(self):
        ...
        for i in range(len(track)):
            ...
            t = self.world.CreateStaticBody(fixtures=self.fd_tile)                     # The call of a static body may be very important and is based on the df_tile = [rl1, rr1, rl2, rr2]
            t.userData = t
            c = 0.01 * (i % 3)                                                         # This is the interchanging colors for the tiles 
    ...
    def step(self, action):
        ...
        self.t += 1.0 / FPS
        self.state = self.SI.generate_image()  #self.state = self.render("state_pixels")
        step_reward = 0
        done = False
    ...
    def render_road(self):
        ...
        vl = pyglet.graphics.vertex_list(
            len(polygons_) // 3, ("v3f", polygons_), ("c4f", colors)            # gl.GL_QUADS, # The // 3 is dividing by 3 but obtaining only the integer value
        )
        vl.draw(gl.GL_QUADS)
    ...
    def render_obstacles(self):
        ...
        v2 = pyglet.graphics.vertex_list(len(self.obstacle_poly) // 3, ('v3f', self.obstacle_poly),('c3B', C)) 
        v2.draw(gl.GL_QUADS)
    ...
    def collision(self):
        ...
        midpoints_upper = [(self.obstacles_pos[:,2,0] + self.obstacles_pos[:,3,0])/2 , (self.obstacles_pos[:,2,1] + self.obstacles_pos[:,3,1])/2]
        midpoints_lower = [(self.obstacles_pos[:,2,0] + self.obstacles_pos[:,3,0])/2 , (self.obstacles_pos[:,2,1] + self.obstacles_pos[:,3,1])/2]

        distance_1 = np.sqrt((x1 - self.obstacles_pos[:,:,0])**2 + (y1 - self.obstacles_pos[:,:,1])**2)
        distance_2 = np.sqrt((x2 - self.obstacles_pos[:,:,0])**2 + (y2 - self.obstacles_pos[:,:,1])**2)
        distance_3 = np.sqrt((x3 - self.obstacles_pos[:,:,0])**2 + (y3 - self.obstacles_pos[:,:,1])**2)
        distance_4 = np.sqrt((x4 - self.obstacles_pos[:,:,0])**2 + (y4 - self.obstacles_pos[:,:,1])**2)

        u_midpoint_1 = np.sqrt((x1 - midpoints_upper[0])**2 + (y1 - midpoints_upper[1])**2) * 0.5
        u_midpoint_2 = np.sqrt((x2 - midpoints_upper[0])**2 + (y2 - midpoints_upper[1])**2) * 0.5
        u_midpoint_3 = np.sqrt((x3 - midpoints_upper[0])**2 + (y3 - midpoints_upper[1])**2) * 0.5
        u_midpoint_4 = np.sqrt((x4 - midpoints_upper[0])**2 + (y4 - midpoints_upper[1])**2) * 0.5

        l_midpoint_1 = np.sqrt((x1 - midpoints_lower[0])**2 + (y1 - midpoints_lower[1])**2) * 0.5
        l_midpoint_2 = np.sqrt((x2 - midpoints_lower[0])**2 + (y2 - midpoints_lower[1])**2) * 0.5
        l_midpoint_3 = np.sqrt((x3 - midpoints_lower[0])**2 + (y3 - midpoints_lower[1])**2) * 0.5
        l_midpoint_4 = np.sqrt((x4 - midpoints_lower[0])**2 + (y4 - midpoints_lower[1])**2) * 0.5
        
        smallest_distance = np.min([np.min(distance_1),   np.min(distance_2),   np.min(distance_3),   np.min(distance_4),
                                    np.min(u_midpoint_1), np.min(u_midpoint_2), np.min(u_midpoint_3), np.min(u_midpoint_4),
                                    np.min(l_midpoint_1), np.min(l_midpoint_2), np.min(l_midpoint_3), np.min(l_midpoint_4)])
