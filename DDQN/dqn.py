...
class DQN:
    ...
    def create_network(self, input, trainable):
        ...
        net = tf.compat.v1.layers.conv2d(inputs=input, filters=8, kernel_size=(7,7), strides=4, name='conv1', kernel_regularizer=wr)
        net = tf.nn.relu(net)
        net = tf.nn.max_pool2d(net, ksize=2, strides=2, padding='SAME')
        net = tf.compat.v1.layers.conv2d(inputs=net, filters=16, kernel_size=(3, 3), strides=1, name='conv2',kernel_regularizer=wr)
        net = tf.nn.relu(net)
        net = tf.nn.max_pool2d(net, ksize=2, strides=2, padding='SAME')
        net = tf.compat.v1.layers.flatten(net)
        net = tf.compat.v1.layers.dense(net, 400, activation=tf.nn.relu, kernel_regularizer=wr)
        # net = tf.layers.dropout(net, 0.5)
        q_state_action_values = tf.compat.v1.layers.dense(net, self.dim_actions, activation=None, kernel_regularizer=wr)
...
#             observation = processimage.process_image(observation)
            ...
            if self.do_training:
                ...
                train_cond = (self.exp_history.counter >= self.min_experience_size and self.global_counter % self.train_freq == 0)
                if train_cond:
                    self.train()
